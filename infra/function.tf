# Function Serverless de autenticação por CPF (Fase 3). Código em
# ../azure-function-auth-cpf. Plano Consumption (Y1): paga só pelas execuções,
# sem custo de infraestrutura ociosa — adequado para uma function de baixo
# volume como essa.
...
# Toda Azure Function precisa de uma Storage Account própria para o runtime
# (controle de triggers, locks, logs) — separada da storage account de
# assinaturas de orçamento (storage.tf) para não misturar as duas
# responsabilidades.
...
resource "azurerm_linux_function_app" "auth_cpf" {
  ...
  app_settings = {
    DB_HOST                = azurerm_postgresql_flexible_server.oficina_db.fqdn
    DB_PORT                = "5432"
    DB_NAME                = "postgres"
    DB_USER                = "adminuser"
