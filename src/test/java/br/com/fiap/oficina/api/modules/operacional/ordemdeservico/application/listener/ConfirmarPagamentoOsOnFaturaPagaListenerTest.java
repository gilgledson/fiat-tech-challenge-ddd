...
class ConfirmarPagamentoOsOnFaturaPagaListenerTest {
    ...
    @InjectMocks
    private ConfirmarPagamentoOsOnFaturaPagaListener listener;

    @Test
    @DisplayName("Deve atualizar OS para PAGA e publicar evento quando recebe fatura paga")
    ...
    void deveLancarExcecaoQuandoOsNaoEncontrada() {
        ...
        JsonObject json = new JsonObject()
                .put("ordemServicoId", osId.toString())
                .put("faturaId", UUID.randomUUID().toString());

        assertThrows(RuntimeException.class, () -> listener.onFaturaPaga(json));
    }
}
