...
class IniciarDiagnosticoUseCaseTest {
    ...
    @InjectMocks
    private IniciarDiagnosticoUseCaseImpl useCase;

    @Test
    @DisplayName("Deve iniciar diagnóstico de uma OS ABERTA com sucesso")
    void deveIniciarDiagnosticoComSucesso() {
        ...
        assertEquals(OrdemDeServicoStatus.EM_DIAGNOSTICO, os.getStatus());
    ...
    @Test
    @DisplayName("Deve lançar exceção quando OS estiver ENTREGUE")
    void deveLancarExcecaoQuandoOsEntregue() {
        UUID osId = UUID.randomUUID();
        OrdemDeServico os = new OrdemDeServico();
        os.setId(osId);
